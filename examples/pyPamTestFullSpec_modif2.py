from __future__ import print_function

import pyPamtra
from copy import deepcopy
import matplotlib.pyplot as plt
import numpy as np

verbosity = 0
plt.figure()

pam = pyPamtra.pyPamtra()
#!name       as_ratio    liq_ice     rho_ms    a_ms    b_ms    alpha    beta   moment_in   nbin      dist_name        p_1     p_2     p_3     p_4     d_1       d_2           scat_name   vel_size_mod           canting
#pam.df.addHydrometeor(('ice', 1.0, -1, 200,-99,-99, np.pi/4., 2, 0, 100, 'exp', 3000, 3e8, -99.0, -99.0, 100e-6,  1000e-6, 'mie-sphere', 'heymsfield10_particles',0.0))
r_snow=0.001
r_ice=50e-6
pam.df.addHydrometeor(("snow",1., -1 , 100., -99,-99, np.pi/4, 2. ,  3 ,1,"mono",-99.0, -99.0, -99.0, -99.0,2*r_snow,-99.0,"ss-rayleigh-gans",'lmdz_snow',0.0))
pam.df.addHydrometeor(("ice", 1., -1 , 917,  -99,-99 ,np.pi/4, 2.  , 3 ,1, "mono", -99., -99., -99., -99., 2*r_ice, -99., "ss-rayleigh-gans", "heymsfield10_particles",0.0))
#pam.df.addHydrometeor(("snow",1., -1 , 100., -99,-99, np.pi/4, 2. ,  3 ,1,"mono",-99.0, -99.0, -99.0, -99.0,2*r_snow,-99.0,"ss-rayleigh-gans",'heymsfield10_particles',0.0))



#pam.df.addHydrometeor(("snow",1., -1 , 100., -99,-99, np.pi/4, 2. ,  3 ,1,"mono",-99.0, -99.0, -99.0, -99.0,2*r_snow,-99.0,"ss-rayleigh-gans",-1.,0.0))

pam = pyPamtra.importer.createUsStandardProfile(pam,hgt_lev=np.arange(1000,1300,200))
pam.p["airturb"][:] = 0.0015
freqs = [35.5]#,80,150]

pam.set["verbose"] = 0
pam.set["pyVerbose"] =0
pam.nmlSet["passive"] = False
pam.nmlSet["randomseed"] = 10
pam.nmlSet["radar_mode"] = "spectrum"
pam.nmlSet["radar_aliasing_nyquist_interv"] = 3
pam.nmlSet["hydro_adaptive_grid"] = False
pam.nmlSet["conserve_mass_rescale_dsd"] = False
pam.nmlSet["radar_use_hildebrand"] = True
pam.nmlSet["radar_noise_distance_factor"] = -2

pam.p["hydro_q"][:] = 0.002
pam.nmlSet["save_psd"] = True

pam.set['verbose'] = verbosity
pam.set['pyVerbose'] = verbosity

# pam.writeNmlFile("test.nml")
# pam.writePamtraProfile("test.lev")

pam.runPamtra(freqs,checkData=False)
plt.plot(pam.r["radar_vel"][0],pam.r["radar_spectra"][0,0,0,0,0])
print(np.shape(pam.r["radar_vel"][0]),np.shape(pam.r["radar_spectra"][0,0,0,0,0]))
plt.show()




print('ref', pam.r["Ze"])
