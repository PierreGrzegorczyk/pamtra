...
#   nb_crystals_bin[:,:,:,i]=nb_crystals_bin[:,:,:,i]*nb_crystals/np.sum(nb_crystals_bin,3) #concentration of ice crystals per bin
    #nb_crystals_bin[:,:,:,i]=1/N_bin*(q_hydro[:,:,:,id_ice]*Rho_air[:,:,:])/(Rho_ice*4/3*np.pi*r_ice**3)
...
for i in range(0,N_bin):
    pam.df.addHydrometeor(("ice_bin_"+str(i), -99., -1 , Rho_ice,  -99., -99. ,-99., -99. , 3 ,1, "mono", -99., -99., -99., -99., 2*r_ice, -99., "mie-sphere", "heymsfield10_particles",0.0))
...
if Run_pamtra==True:
    ...
    plt.figure('test')
    plt.imshow(pam.r["Ze"][:,0,:,0,0,0].T,aspect='auto',vmin=-20,vmax=30,cmap="plasma")
    plt.colorbar()
    print("Run end")
    plt.show()
