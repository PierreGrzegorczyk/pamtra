...
# linear spaced bins
dD=150
Bin_center=1e-6*(np.arange(0,N_bin,1)*dD+dD)
Bin_edges=1e-6*(np.arange(0,N_bin+1,1)*dD+dD/2)
...
print('Ice bin',T[65,0,26],nb_crystals[65,0,26],np.sum(nb_crystals_bin[65,0,26,:],-1))
...
##___ICE
#for i in range(0,N_bin):
print('bin edges', nb_crystals_bin[65,0,26,:])
for i in range(0,9):
    ...
    pam.df.dataFullSpec["n_ds"][:,:,:,globals()["id_ice_"+str(i)],0]=nb_crystals_bin[:jsel,:,:,i]
    pam.df.dataFullSpec["mass_ds"][:,:,:,globals()["id_ice_"+str(i)],0]=q_hydro[:jsel,:,:,globals()["id_ice_"+str(i)]]
